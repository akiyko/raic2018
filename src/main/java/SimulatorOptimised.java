 ...
 */
public class SimulatorOptimised {
...
//        update(delta_time / MICROTICKS_PER_TICK)
    ...
    public static void update(double delta_time, Rules rules, List<MyRobot> robots, MyBall ball, OptimiseOptions opt) {
...
//      }
    ...
    public static void tick(Rules rules, List<MyRobot> robots, MyBall ball, OptimiseOptions opt) {
        tick(rules, robots, ball, Constants.MICROTICKS_PER_TICK, opt);

    }

    public static void tick(Rules rules, List<MyRobot> robots, MyBall ball, double microticksPerTick, OptimiseOptions opt) {
        double delta_time = 1.0 / Constants.TICKS_PER_SECOND;
        for( int i = 0; i< microticksPerTick; i++) {
            update(delta_time / microticksPerTick, rules, robots, ball, opt);
        }
//        for pack in nitro_packs:
//        if pack.alive:
//        continue
